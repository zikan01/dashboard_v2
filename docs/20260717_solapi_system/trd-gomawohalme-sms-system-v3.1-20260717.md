...
실제 데이터 확인 결과 (2026-07-17): 하이픈 포함 형식(`010-1234-5678`)과
**별표 마스킹 값**(`****...`)이 함께 존재한다.

마스킹의 원인: 네이버는 방문일이 지나면 예약자 번호를 마스킹한다.
따라서 마스킹 번호는 방문일 경과 후 내려받은 엑셀에서만 유입되며,
**방문일 전에 업로드하면 정상 번호가 확보된다.**

- 정규화 후 숫자만 남겨 `01X`로 시작하는 10~11자리가 아니면 무효로 판정하고,
  외부 API를 호출하지 않은 채 `전화번호 오류`로 분류해 실패 관리에 노출한다.
- 한 번 확보된 정상 번호는 마이그레이션 0004의 가드(`apply_import_plan`)가
  보호하므로, 방문일 이후 재업로드에도 마스킹 값으로 덮어써지지 않는다.
- 자동 안내(D-7~당일)는 모두 방문 전 발송이므로, 방문일 전 업로드를
  운영 권장사항으로 한다.
...
```text
supabase/migrations/
├─ 0005_notification_core.sql
├─ 0006_notification_functions.sql
├─ 0007_notification_rls.sql
├─ 0008_notification_cron.sql
└─ 0009_seed_notification_defaults.sql
```

- 0005: 컬럼·설정·템플릿·규칙·Job·Delivery·Webhook 테이블
- 0006: 일정 생성·Claim·상태 계산·잠금 복구 함수와 Trigger
- 0007: RLS
- 0008: Cron
- 0009: 기본 D-7·D-3·D-1·당일 규칙

0001~0003은 기존 대시보드, 0004는 연락처 유실 방지 가드(2026-07-17)가
사용하므로 발송 시스템은 0005부터 시작한다.
